...
APP_PORT="10009"   # ✅ FIXED (internal port)
JAR_PATH="target/*.jar"
LOG_DIR="/var/log/resume-service"
LOG_FILE="$LOG_DIR/resume-service.log"
...
sshpass -p "$PASSWORD" ssh -o StrictHostKeyChecking=no ubuntu@$SERVER_IP << EOF
    ...
    if [ -d "$REPO_NAME" ]; then
        ...
        git clone git@github.com:Rsharma0374/resume-tailor.git
        git -C $REPO_NAME checkout $BRANCH
    fi
