...
echo "=== Starting Document Utility Service Deployment (Build on Jenkins, Run on Server) ==="
...
APP_NAME="document-utility-service"
APP_PORT="10005"
...
sshpass -p "$PASSWORD" ssh -o StrictHostKeyChecking=no "${REMOTE_USER}@${SERVER_IP}" << EOF
  ...
  nohup java -DHOSTNAME="$HOSTNAME" -jar "${REMOTE_JAR_PATH}" --server.port=${APP_PORT} 2>&1 &

  echo "=== Waiting for app to boot ==="
  sleep 20

  echo "=== Health Check ==="
  curl -f "http://127.0.0.1:${APP_PORT}/doc-service/welcome" || echo "Health check failed"
