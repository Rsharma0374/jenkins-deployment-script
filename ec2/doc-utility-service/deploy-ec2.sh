...
echo "=== Starting Document Utility Service Deployment (Maven) ==="
...
APP_PORT="10005"
JAR_PATH="target/*.jar"
LOG_DIR="/var/log/document-utility-service"
LOG_FILE="$LOG_DIR/document-utility-service.log"
...
sshpass -p "$PASSWORD" ssh -o StrictHostKeyChecking=no ubuntu@$SERVER_IP << EOF
    ...
    if [ -d "$REPO_NAME" ]; then
        ...
        git clone git@github.com:Rsharma0374/document-utility-core.git
        git -C $REPO_NAME checkout $BRANCH
    fi
